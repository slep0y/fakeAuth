let form = document.querySelector('#authForm'),
    ...
    formMessage = form.querySelector('.form__message'),
    userAuthorize = document.querySelector('.userAuthorize');
...
function checkAuthData(){
    if(localStorage.getItem('authorize') && localStorage.getItem('userAuth')){
        ...
        if(usersBase[authName]){
            ...
            displayUserName(authName, userAuthorize);
            cleanFields();
        }else{
...
function authorize(e){
    ...
    if(localStorage.getItem('userAuth')){
        ...
        if(usersBase[name] && pass == usersBase[name]['pass']){
            ...
            showMessage('Неверные: Имя и/или Пароль');
        }
    }else{
        showMessage('Неверные: Имя и/или Пароль');
    }
...
function displayUserName(name, whereShow){
    whereShow.innerHTML = name;
}
